...
writeFileSync(join(__dirname, 'src/Pages/BadgePage.jsx'), `import React, { useState, useRef, useCallback } from 'react';
...
export default function BadgePage() {
  ...
  const submitBadge = useMutation({
    ...
    onSuccess: () => {
      ...
      setSubmitName('');
      setSubmitPhoto(null);
      toast.success('Votre soutien a été ajouté au mur !');
    },
  ...
  const handleSubmitFileChange = (e) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = (ev) => setSubmitPhoto(ev.target.result);
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSubmitToWall = (e) => {
    e.preventDefault();
    if (!submitName.trim() || !submitPhoto) {
      toast.error('Veuillez entrer votre nom et choisir une photo.');
      return;
    }
    submitBadge.mutate({ nom: submitName.trim(), photo: submitPhoto });
  };
      ...
      <section className="py-16 bg-slate-50">
        <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            {/* Badge preview */}
            <div ref={badgeRef} style={{ width: 400, height: 480, position: 'relative', overflow: 'hidden', borderRadius: 16, background: '#2d4a5a', fontFamily: 'sans-serif', flexShrink: 0 }}>
            ...
            {/* Action bar */}
            <div className="mt-6 w-full max-w-[420px] flex items-center justify-between bg-blue-950 rounded-2xl px-4 py-3 gap-2">
            ...
            {userPhoto && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6 w-full max-w-[420px]">
                ...
                <div className="mt-4 text-center">
                  <button onClick={() => setShowSubmitForm(true)}
                    className="text-green-600 hover:text-green-700 text-sm font-semibold underline underline-offset-2">
                    Ajouter ma photo au Mur des soutiens →
                  </button>
                </div>
              </motion.div>
            )}
            ...
            {showSubmitForm && (
              <motion.form initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                onSubmit={handleSubmitToWall}
                className="mt-6 w-full max-w-[420px] bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                <h3 className="font-extrabold text-blue-950 text-lg mb-4">Rejoindre le Mur des Soutiens</h3>
                <div className="mb-4">
                  ...
                  <input type="text" value={submitName} onChange={e => setSubmitName(e.target.value)}
                    ...
                    className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-950" />
                </div>
                <div className="mb-5">
                  <label className="block text-sm font-medium text-slate-700 mb-1">Votre photo</label>
                  <input ref={submitFileRef} type="file" accept="image/*" onChange={handleSubmitFileChange} className="hidden" />
                  <button type="button" onClick={() => submitFileRef.current?.click()}
                    className="w-full border-2 border-dashed border-slate-300 hover:border-blue-950 rounded-xl py-3 text-sm text-slate-500 hover:text-blue-950 transition-colors">
                    {submitPhoto ? 'Photo sélectionnée ✓ (cliquer pour changer)' : 'Cliquer pour choisir une photo'}
                  </button>
                </div>
                <div className="flex gap-3">
                  <button type="submit" disabled={submitBadge.isPending}
                    className="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-xl text-sm transition-colors disabled:opacity-50">
                    {submitBadge.isPending ? 'Envoi...' : 'Publier mon soutien'}
                  </button>
...
`, 'utf8');
console.log('BadgePage.jsx written OK');
